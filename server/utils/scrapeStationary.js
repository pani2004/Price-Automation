...
import scraperConfig from './stationaryScraperConfig.json' assert { type: 'json' };

export const scrapeStationary = async (query) => {
  const websites = Object.keys(scraperConfig);
  const results = [];
  const browser = await chromium.launch({ headless: false });

  console.log(`Starting stationary scrape for query: "${query}"`);
  ...
  await browser.close();
  console.log('Stationary scraping complete.');
  return results;
};
